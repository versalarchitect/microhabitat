...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seo = await getPageSEO('terms-of-service', locale as Locale);
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.microhabitat.com';
  const canonicalUrl = locale === 'en' ? `${siteUrl}/terms-of-service` : `${siteUrl}/${locale}/terms-of-service`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        'fr': `${siteUrl}/fr/conditions-utilisation`,
        'de': `${siteUrl}/de/nutzungsbedingungen`,
        'nl': `${siteUrl}/nl/algemene-voorwaarden`,
        'it': `${siteUrl}/it/termini-servizio`,
        'es': `${siteUrl}/es/terminos-servicio`,
      },
    },
    openGraph: {
      title: seo.metaTitle,
...
export default async function TermsOfServicePage({ params }: PageProps) {
  ...
  return <TermsOfServiceClient locale={locale as Locale} translations={translations} />;
}
