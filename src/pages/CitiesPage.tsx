...
export function CitiesPage({ onBookDemo }: CitiesPageProps) {
  ...
  const { data: seo } = useQuery({
    queryKey: queryKeys.pageSEO('cities', locale),
    queryFn: () => getPageSEO('cities', locale),
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <>
      <SEO
        title={seo?.metaTitle || 'Cities | MicroHabitat'}
        description={seo?.metaDescription || 'Discover MicroHabitat urban farms across North America and Europe. We operate in 20+ cities bringing sustainable agriculture to urban spaces.'}
